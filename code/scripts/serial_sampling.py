"""
Serial Sampling Implementation for Adaptive Code Generation

This module implements the serial sampling approach for code generation.
In serial sampling, each generation builds upon previous generations at a given temperature,
allowing for the exploration of related solution paths.

Key features:
- Sampling based on previous solutions in a sequence
...
from llm.prompts.code_generation import get_generic_question_template, get_serial_generation_template, PromptConstants
...
class SerialSampler:
    """
    Implements serial sampling strategy for code generation.
    
    Each sample builds upon previous generations, using them as context
    to generate new solutions at specified temperatures.
    """
    ...
    def generate_solution_serial(self, problem: Dict, temperature: float, depth=10):
        """
        Generate solutions in a serial manner, each building on previous generations.
        
        Args:
            problem: The problem data
            temperature: The temperature for generation
            depth: The number of serial generations to perform
        ...
        for i in range(depth):
            if i == 0:
                ...
                curr_solution, _ = self.generate_solution(problem, temperature=temperature)
            else:
                # Subsequent solutions use previous solutions as context
                prompt = get_serial_generation_template(problem, solutions)
                messages = [
                    {"content": PromptConstants.SYSTEM_MESSAGE_GENERIC, "role": "system"}, 
                    {"content": prompt, "role": "user"}
                ]
                curr_solution = self.completion(messages=messages, temperature=temperature)
    ...
    def process_single_sample(self, problem: Dict, temperature: float) -> Tuple[Optional[List[bool]], float]:
        ...
        """
        solutions, exec_time = self.generate_solution_serial(problem, temperature)
...
def main():
    """Main function to run the serial sampling experiment"""
    # Configuration
    ...
    sampler = SerialSampler(
        ...
        experiment_name="serial_sampling_experiment",
        max_workers=5,
        model="gpt-4o-mini"  # Can be changed to other models
    )

    # Process problems from LCB dataset
